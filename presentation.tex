...
$$
y = 
 \theta_0 + \theta_1x + \theta_2x^2 + ... + \theta_dx^d + \sum_{k=1}^K \theta_{dk}(x-\kappa_k)_+^d 
$$
\vspace{-3.5em}
\end{minipage}
...
\begin{overprint}
\begin{flushleft}
\onslide<1>{Control wiggliness:}\onslide<2>{\hspace{-7.6em}e.\,g.\ first order differences $\xi^TK\xi = \sum(\xi_{k+1} - \xi_{k})^2$:}\onslide<3>{\hspace{-22.1em} Problem: How to choose $\lambda$?}
\end{flushleft}
\vspace{-2em}
\end{overprint}
