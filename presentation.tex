...
\only<5->{
\textcolor{beamer@postercolour}{log-Posterior: } $$l_p(\beta,b|y) \propto l(y,\beta,b) - \sum_{j=1}^p \overbrace{\frac{1}{2\tau_j^2}}^{=\lambda} b_j^T  b_j $$
}
\end{minipage}
...
\begin{itemize}
\item \textbf{Estimate $\boldsymbol{\hat{\beta}_\lambda}$ in inner PIRLS loop}
\end{itemize}
