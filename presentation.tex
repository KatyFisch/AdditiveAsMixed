...
\begin{enumerate}
\item Update  $\boldsymbol{\beta}$ and $\mathbf{b}$ given the current $\lambda$
\item Update $\lambda$ using Fisher-Scoring (or Newton-Raphson)
...
\begin{itemize}
\item For each $\lambda$: Get estimates for $\boldsymbol{\beta}$ and $\mathbf{b}$: 
